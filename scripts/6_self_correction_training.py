...
class Config:
    ...
    SC_DATA_ORIGINAL = {
        "zh_en": {
            ...
            "train": "../../data/processed/zh_en/self_correction_zh_en_70-30/train.tsv",
            "val": "../../data/processed/zh_en/self_correction_zh_en_70-30/val.tsv"
        },
        "de_en": {
    ...
    # from WMT-merged model
    WMT_MODEL_PATH = f"../models/wmt_merged/{MODEL_TYPE}_{LANG_PAIR}"
    
    SUFFIX = "_end" if USE_END_TOKEN else ""
    OUTPUT_DIR = f"../models/self_correction/{MODEL_TYPE}_{LANG_PAIR}{SUFFIX}_70_30"
    ...
    LEARNING_RATE = 1e-4
    NUM_EPOCHS = 3
    MAX_LENGTH = 768  # longer for self-correction format
    WARMUP_RATIO = 0.05
    SEED = 42
...
def load_data(train_path, val_path):    
    def read_tsv(path):
        ...
        # expect 'prompt' and 'completion' columns
...
def main():
    ...
    print(f"Self-Correction Fine-tuning")
