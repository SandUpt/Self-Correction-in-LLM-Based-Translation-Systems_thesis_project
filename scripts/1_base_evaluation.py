...
class Config:
    ...
    # evaluation settings
    SAMPLE_SIZE = None  # None for full eval
    MAX_NEW_TOKENS = 256
    # batch 32 OOMs on A100 40GB with llama3, same with llama 2 but works fine with mistral and qwen
    BATCH_SIZE = 48
    SEED = 42
...
def generate_translations(model, tokenizer, sources, prompt_template):
    ...
    for i in tqdm(range(0, len(sources), Config.BATCH_SIZE), desc="Generating"):
        ...
        for j, output in enumerate(outputs):
            ...
            # clean up the output - base models be messy
...
def clean_output(text):
    """Extracting just the translation from model output"""
    if not text:
        return ""
    
    text = text.strip()
    
    # take first line only cause base models sometimes keep generating
    text = text.split("\n")[0].strip()
    
    # remove common artifacts stuff
...
def compute_metrics(predictions, references, sources, comet_model):
    ...
    # switched to wmt22 from wmt20 for better correlation
