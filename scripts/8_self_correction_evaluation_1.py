...
class Config:
    ...
    OUTPUT_DIR = f"../evaluations/self_correction/{MODEL_TYPE}_{LANG_PAIR}{SUFFIX}_70_30"
...
def extract_translations(text):
    """
    Extracting initial and corrected translations from self-correction output
    For repetition, cuttt off at second occurrence of any marker.
    """
    result = {
        "initial": None,
        "analysis": None,
...
def clean_translation(text):
    ...
    # cut off at ||| (end marker or repeated pipes)
    pipe_idx = text.find("|||")
    if pipe_idx > 0:
...
def main():
    ...
    # get final translations (corrected, else initial)
