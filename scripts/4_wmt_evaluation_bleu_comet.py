...
class Config:
    ...
    MODEL_PATH = f"../models/wmt_merged/{MODEL_TYPE}_{LANG_PAIR}"
    OUTPUT_DIR = f"../evaluations/wmt/{MODEL_TYPE}_{LANG_PAIR}"
...
class StopOnEnd(StoppingCriteria):
    """Stop generation when |||END||| is produced, handle variants with space or newline"""
    END_VARIANTS = (
        "|||END|||", " |||END|||", "\n|||END|||",
        "||| END |||", " ||| END |||", "\n||| END |||"
    )
...
def clean_output(text):
    """Extract translation from model output, removing end markers and artifacts"""
    if not text:
        return ""
    
    text = text.strip()
    
    # cut off at first occurrence of ||| (start of end marker or repeated pipes)
    pipe_idx = text.find("|||")
    if pipe_idx > 0:
        text = text[:pipe_idx].strip()
    
    # handle case where model outputs repeated | without space
    pipe_idx = text.find("||")
    if pipe_idx > 0:
        text = text[:pipe_idx].strip()
    
    # remove any remaining pipe characters at the end
...
def main():
    ...
    # print a few samples to verify output quality
    print("\nSample outputs:")
