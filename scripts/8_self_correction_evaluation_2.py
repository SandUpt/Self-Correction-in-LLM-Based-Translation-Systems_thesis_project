...
class Config:
    ...
    MODEL_PATH = f"../models/self_correction_merged/{MODEL_TYPE}_{LANG_PAIR}{SUFFIX}_70_30"
    OUTPUT_DIR = f"../evaluations/self_correction/{MODEL_TYPE}_{LANG_PAIR}{SUFFIX}_70_30_eval2"
    
    # evaluation settings
    SAMPLE_SIZE = None
    MAX_NEW_TOKENS = 400  # longer for self-correction output
    # batch 32 OOMs on A100 40GB with llama3
    BATCH_SIZE = 32  # smaller batch for longer outputs
    SEED = 42
...
def extract_translations(text):
    """
    Extract initial and corrected translations from self-correction output.
    Handles repetition by cutting off at second occurrence of any marker.
    """
    result = {
        "initial": None,
        "analysis": None,
    ...
    # cut at common continuation patterns (model keeps generating after it should stop)
    stop_patterns = [
        "Human:",
    ...
    # find first occurrence of each marker
...
def clean_translation(text):
    """Clean extracted translation, handling repetition patterns"""
    if not text:
        return ""
    
    text = text.strip()
    
    # cut at ||| (end marker or repeated pipes)
    pipe_idx = text.find("|||")
    if pipe_idx > 0:
    ...
    # detect sentence-level repetition, if same sentence appears twice, keep only first occurrence
    ...
    if len(sentences) > 1:
        ...
        for sent in sentences:
            ...
            elif sent_normalized in seen:
                # repetition detected, stop here
                break
        text = ' '.join(unique_sentences)
    
    # also check for phrase repetition (same 6+ words appearing twice)
    words = text.split()
    if len(words) > 12:
        for phrase_len in range(6, min(15, len(words)//2)):
            for i in range(len(words) - phrase_len * 2):
                ...
                if phrase in rest:
                    # found repetition, cut at end of first occurrence
...
def compute_per_example_metrics(predictions, references, sources, comet_model):
    """Compute BLEU and COMET for each example"""
    bleu = BLEU(effective_order=True)  # suppress warning for short sentences
...
def main():
    ...
    print(f"Self-Correction Model Evaluation")
    ...
    # get final translations (corrected if available, else initial)
    ...
    # analyse improvements/degradations
    bleu_improved = 0
    bleu_degraded = 0
    bleu_unchanged = 0
    comet_improved = 0
