...
class Config:
    ...
    MODEL_PATH = f"../models/wmt_merged/{MODEL_TYPE}_{LANG_PAIR}_4M_Lora"
    OUTPUT_DIR = f"../evaluations/wmt/{MODEL_TYPE}_{LANG_PAIR}_4M_Lora"
...
class StopOnEnd(StoppingCriteria):
    """Stop generation when |||END||| is produced (for variants with space/newline)"""
    END_VARIANTS = (
        "|||END|||", " |||END|||", "\n|||END|||",
        "||| END |||", " ||| END |||", "\n||| END |||"
    )
...
def clean_output(text):
    ...
    # case where model outputs repeated | without space
    pipe_idx = text.find("||")
    if pipe_idx > 0:
        text = text[:pipe_idx].strip()
    
    #  any remaining pipe characters at the end
    text = text.rstrip("|").strip()
    
    # special tokens that might leak 
    text = re.sub(r'<\|.*?\|>', '', text)
